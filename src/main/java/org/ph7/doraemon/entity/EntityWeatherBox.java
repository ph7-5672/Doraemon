...
public class EntityWeatherBox extends EntityItemBase
{
    ...
    public boolean processInitialInteract(EntityPlayer player, EnumHand hand)
    {
        ...
        if (mc.entityRenderer instanceof WeatherRenderer)
        {
            WeatherRenderer renderer = (WeatherRenderer)mc.entityRenderer;
            renderer.weather = newWeather;
        }
    ...
    protected EnumWeather getWeather()
    {
        Integer meta = this.dataManager.get(WEATHER);
        return EnumWeather.getByMeta(meta);
    }

    protected void setWeather(EnumWeather weather)
    {
    ...
    public static class WeatherRenderer extends EntityRenderer
    {
        ...
        public WeatherRenderer(Minecraft mcIn, IResourceManager resourceManagerIn)
        {
        ...
        {
            if (this.weather != EnumWeather.SNOW && this.weather != EnumWeather.RAIN_SNOW)
            {
                this.renderRainSnowSuper(partialTicks);
                return;
            }
                ...
                for (int l1 = i - i1; l1 <= i + i1; ++l1)
                {
                    ...
                    if (biome.canRain() || biome.getEnableSnow())
                    {
                        ...
                        if (k2 != l2)
                        {
                            ...
                            if (this.weather == EnumWeather.RAIN_SNOW)
                            {
                                if (j1 != 0)
                                {
                                    if (j1 >= 0)
        ...
        private void renderRainSnowSuper(float partialTicks)
        {
            net.minecraftforge.client.IRenderHandler renderer = this.mc.world.provider.getWeatherRenderer();
            if (renderer != null)
            {
